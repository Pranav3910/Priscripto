...
const MyProfile = () => {
  ...
  return (
    ...
    <div className='mt-10'>
      {isEdit
        ? <button className='cursor-pointer border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all' onClick={()=>setIsEdit(false)}>Save Information</button>
        : <button className='cursor-pointer border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all' onClick={()=>setIsEdit(true)}>edit</button>
      }
    </div>
    </div>
  )
